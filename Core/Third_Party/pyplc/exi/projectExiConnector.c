

#include "projectExiConnector.h"
#include <string.h> /* for memcpy */






//ssId sessionId;
//uint8_t sessionId[SESSIONID_LEN];
//uint8_t sessionIdLen;


#if defined(__cplusplus)
extern "C"
{
#endif
void addToTrace_chararray(char *s);
#if defined(__cplusplus)
}
#endif

void debugAddStringAndInt(char *s, int i) {
	//char sTmp[1000];
	//McuXFormat_xsprintf(sTmp, "%s%d", s, i);
    /* todo: before appending, check the size. Otherwise we get overflow! */
	//strcat(gDebugString, sTmp);
}


int projectExiConnector_decode_appHandExiDocument(projectExiConnector* const me) {
  /* precondition: The global_streamDec.size and global_streamDec.data have been set to the byte array with EXI data. */

	me->global_streamDec.pos = &me->global_streamDecPos;
  *(me->global_streamDec.pos) = 0; /* the decoder shall start at the byte 0 */
  me->g_errn = decode_appHandExiDocument(&me->global_streamDec, &me->aphsDocDec);
  return me->g_errn;
}

int projectExiConnector_decode_DinExiDocument(projectExiConnector* const me) {
  /* precondition: The global_streamDec.size and global_streamDec.data have been set to the byte array with EXI data. */

  me->global_streamDec.pos = &me->global_streamDecPos;
  *(me->global_streamDec.pos) = 0; /* the decoder shall start at the byte 0 */
  me->g_errn = decode_dinExiDocument(&me->global_streamDec, &me->dinDocDec);
  return me->g_errn;
}

#ifdef NOT_USED
void projectExiConnector_testEncode(projectExiConnector* const me) {
	projectExiConnector_prepare_DinExiDocument();
	dinDocEnc.V2G_Message.Body.SessionSetupReq_isUsed = 1u;
	init_dinSessionSetupReqType(&dinDocEnc.V2G_Message.Body.SessionSetupReq);
	/* In the session setup request, the session ID zero means: create a new session.
	   The format (len 8, all zero) is taken from the original Ioniq behavior. */
	dinDocEnc.V2G_Message.Header.SessionID.bytes[0] = 0;
	dinDocEnc.V2G_Message.Header.SessionID.bytes[1] = 0;
	dinDocEnc.V2G_Message.Header.SessionID.bytes[2] = 0;
	dinDocEnc.V2G_Message.Header.SessionID.bytes[3] = 0;
	dinDocEnc.V2G_Message.Header.SessionID.bytes[4] = 0;
	dinDocEnc.V2G_Message.Header.SessionID.bytes[5] = 0;
	dinDocEnc.V2G_Message.Header.SessionID.bytes[6] = 0;
	dinDocEnc.V2G_Message.Header.SessionID.bytes[7] = 0;
	dinDocEnc.V2G_Message.Header.SessionID.bytesLen = 8;
	projectExiConnector_encode_DinExiDocument();
}
#endif

void projectExiConnector_prepare_DinExiDocument(projectExiConnector* const me) {
	/* before filling and encoding the dinDocEnc, we initialize here all its content. */
	init_dinEXIDocument(&me->dinDocEnc);
	me->dinDocEnc.V2G_Message_isUsed = 1u;
	init_dinMessageHeaderType(&me->dinDocEnc.V2G_Message.Header);
	init_dinBodyType(&me->dinDocEnc.V2G_Message.Body);
	/* take the sessionID from the global variable: */
	memcpy(me->dinDocEnc.V2G_Message.Header.SessionID.bytes, me->sessionId.u8t, SESSIONID_LEN);
	me->dinDocEnc.V2G_Message.Header.SessionID.bytesLen = SESSIONID_LEN;//sessionIdLen
}

int projectExiConnector_encode_DinExiDocument(projectExiConnector* const me) {
  /* precondition: dinDocEnc structure is filled. Output: global_stream.data and global_stream.pos. */  
	me->global_streamEnc.size = EXI_TRANSMIT_BUFFER_SIZE;
	me->global_streamEnc.data = me->exiTransmitBuffer;
	me->global_streamEnc.pos = &me->global_streamEncPos;
	*(me->global_streamEnc.pos) = 0; /* start adding data at position 0 */
	me->g_errn = encode_dinExiDocument(&me->global_streamEnc, &me->dinDocEnc);
	return me->g_errn;
}

void projectExiConnector_prepare_appHandExiDocument(projectExiConnector* const me) {				//evse added
	/* before filling and encoding the dinDocEnc, we initialize here all its content. */
	init_appHandEXIDocument(&me->aphsDocEnc);
	//dinDocEnc.V2G_Message_isUsed = 1u;
	//init_appHandAppProtocolType(appHandAppProtocolType);
	//init_dinMessageHeaderType(&aphsDoc.V2G_Message.Header);
	//init_dinBodyType(&dinDocEnc.V2G_Message.Body);
	/* take the sessionID from the global variable: */
	//memcpy(dinDocEnc.V2G_Message.Header.SessionID.bytes, sessionId, SESSIONID_LEN);
	//dinDocEnc.V2G_Message.Header.SessionID.bytesLen = sessionIdLen;
}


void projectExiConnector_encode_appHandExiDocument(projectExiConnector* const me) {
	  /* precondition: dinDocEnc structure is filled. Output: global_stream.data and global_stream.pos. */
	me->global_streamEnc.size = EXI_TRANSMIT_BUFFER_SIZE;
	me->global_streamEnc.data = me->exiTransmitBuffer;
	me->global_streamEnc.pos = &me->global_streamEncPos;
	*(me->global_streamEnc.pos) = 0; /* start adding data at position 0 */
	me->g_errn = encode_appHandExiDocument(&me->global_streamEnc, &me->aphsDocEnc);
}






#ifdef NOT_USED
int projectExiConnector_test(int a) {
  unsigned int i;
  char s[100];
  char s2[100];
  strcpy(gDebugString, "");
  global_streamDec.size = mytestbufferLen;
  global_streamDec.data = mytestbuffer;
  global_streamDec.pos = &global_streamDecPos;
  *(global_streamDec.pos) = 0; /* the decoder shall start at the byte 0 */	
  
  for (i=0; i<1000; i++) { /* for runtime measuremement, run the decoder n times. */
    g_errn = decode_dinExiDocument(&global_streamDec, &dinDocDec);
  }
  //dinDocDec.V2G_Message.Header.SessionID.bytesLen
  addToTrace_chararray("Test from projectExiConnector_test");
  McuXFormat_xsprintf(s, "SessionID ");
  for (i=0; i<4; i++) {
	McuXFormat_xsprintf(s2, "%hx ", dinDocDec.V2G_Message.Header.SessionID.bytes[i]);
	strcat(s, s2);
  }
  addToTrace_chararray(s);  
  //g_errn = encode_dinExiDocument(&global_streamEnc, &dinDocEnc);
 return 2*a;
}
#endif
