...
extern PCD_HandleTypeDef hpcd_USB_FS;	//from main.c
...
USBD_StatusTypeDef USBD_LL_Init(USBD_HandleTypeDef *pdev)
{
  ...
  if (pdev->id == DEVICE_FS)
  {
    ...
    uint16_t pma_track = (NUMBER_OF_CDC*2*8U+16U);//0x70;//(NUMBER_OF_CDC*2*8U+16U); //0x40; /** PMA offset*///0x48

    HAL_PCDEx_PMAConfig((PCD_HandleTypeDef *)pdev->pData, 0x00, PCD_SNG_BUF, pma_track);	//TX Ep 0 Buffer
    pma_track += 0x40;

    HAL_PCDEx_PMAConfig((PCD_HandleTypeDef *)pdev->pData, 0x80, PCD_SNG_BUF, pma_track);	//RX Ep 0 Buffer
    ...
    for (uint8_t i = 0; i < USBD_CDC_ACM_COUNT; i++)
    {
      HAL_PCDEx_PMAConfig((PCD_HandleTypeDef *)pdev->pData, CDC_IN_EP[i], PCD_SNG_BUF, pma_track);	//TX Ep 1 Buffer
      pma_track += 0x40;//48

      HAL_PCDEx_PMAConfig((PCD_HandleTypeDef *)pdev->pData, CDC_OUT_EP[i], PCD_SNG_BUF, pma_track);	//RX Ep 1 Buffer
      pma_track += 0x40;//48

      HAL_PCDEx_PMAConfig((PCD_HandleTypeDef *)pdev->pData, CDC_CMD_EP[i], PCD_SNG_BUF, pma_track);	//RX Ep 2 Buffer
      pma_track += 0x10;//8
    }
#endif
